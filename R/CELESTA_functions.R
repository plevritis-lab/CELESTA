...
Celesta <- setClass("Celesta",
                    slots = c(marker_exp_matrix = "matrix", # normalized expression from segmentation
                              ...
                              marker_activation_prob = "matrix", # marker expression probability
                              ...
                              cells_to_check = "numeric", # store cells to check in each round and each iteration
                              cells_with_ID = "numeric")) # cells already assigned cell type
...
#' @export
marker_activation_probability <- function(CelestaObj,figure=FALSE){
  ...
  marker_activation_prob <- matrix(nrow=dim(marker_exp_matrix)[1],ncol=dim(marker_exp_matrix)[2])
  colnames(marker_activation_prob) <- colnames(marker_exp_matrix)
  
  for(i in 1:dim(marker_exp_matrix)[2]){
    exp_term <- exp(sigmoid_function_parameter[2,i]*(marker_exp_matrix[,i]-sigmoid_function_parameter[1,i]))
    y = exp_term/(1+exp_term)
    marker_activation_prob[,i] <- (y-min(y))/(max(y)-min(y))
  }
  CelestaObj@marker_activation_prob <- marker_activation_prob
  return(CelestaObj)
}
#############################################################################################
...
CreateCELESTAobj <- function(project_title="Project",prior_marker_info,imaging_data_file,
  ...
  CelestaObj <- marker_activation_probability(CelestaObj)
...
cell_filtering <- function(high_marker_threshold=0.9, low_marker_threshold=0.4,
  ...
  for(i in 1:dim(CelestaObj@marker_exp_matrix)[1]){
    cell_activation_prob <- CelestaObj@marker_activation_prob[i,]
...
#' Find cells to check
#' @export
find_cells_to_check <- function(CelestaObj,round){
  current_cell_type_assignment <- CelestaObj@current_cell_type_assignment
  lineage_info <- CelestaObj@lineage_info
  cell_ID <- CelestaObj@cell_ID
  if(round == 1){
    cells_to_check <- cell_ID[which(current_cell_type_assignment[,round] == 0)]
  }else{
...
#' @export
find_cells_with_ID <- function(CelestaObj,round){
  current_cell_type_assignment <- CelestaObj@current_cell_type_assignment
  lineage_info <- CelestaObj@lineage_info
  cell_ID <- CelestaObj@cell_ID
  if(round == 1){
    cells_with_ID <- cell_ID[which(current_cell_type_assignment[,round] != 0 & 
...
#' @export
scoring_function <- function(CelestaObj,round,cells_to_check,cell_type_num){
  marker_activation_prob <- CelestaObj@marker_activation_prob
  ...
  activation_prob_to_use <- marker_activation_prob[cells_to_check,]
  for(i in 1:length(cell_type_num)){
    non_NA_index <- which(!is.na(current_pri_matrix[i,]))
    prior_info <- current_pri_matrix[i,non_NA_index]
    current_scoring_matrix[cells_to_check,cell_type_num[i]] <- get_score(activation_prob_to_use,prior_info,non_NA_index)
  }
  current_scoring_matrix[cells_to_check,cell_type_num]<-t(apply(current_scoring_matrix[cells_to_check,cell_type_num],
...
#' @export
find_min_prob_diff <- function(max.prob,max.prob_index,cell_prob_list,cells_to_check){
  ### max.prob, max.prob_index are calculated only on cells_to_check
  ### but cell_prob_list has all the cells
  min_prob_diff <- numeric(length=length(cells_to_check))
  for(i in 1:length(cells_to_check)){
    min_prob_diff[i] <- min(max.prob[i]-cell_prob_list[cells_to_check[i],-max.prob_index[i]])
  }
...
#' @export
cell_type <- function(CelestaObj,cell_type_num,cells_to_check,round,
  ...
  marker_activation_prob <- CelestaObj@marker_activation_prob
  max.prob_index <- apply(cell_prob_list[cells_to_check,],1,which.max)
  max.prob <- apply(cell_prob_list[cells_to_check,],1,max)
  min_prob_diff <- find_min_prob_diff(max.prob,max.prob_index,cell_prob_list,cells_to_check)
  ### Find cells with cell type max probability > threshold and cell type probability difference > threshold
  ########################################
  ### Indexing on cells_to_check!!!!!!!!
  threshold_cells <- cells_to_check[which(min_prob_diff > min_difference & max.prob > min_prob)]
  ...
  for(i in 1:length(threshold_cells)){
    ...
    if(length(which(marker_activation_prob[cell_ID_to_check,high_marker_index]>=high_marker_threshold[threshold_index]))==length(high_marker_index) &
       length(which(marker_activation_prob[cell_ID_to_check,low_marker_index]<=low_marker_threshold[threshold_index]))==length(low_marker_index)){
...
#       cells_to_check <- which(current_cell_type_assignment == cell_number_to_use[i])
#       cell_index[(count+1):(count+length(cells_to_check))] <- cells_to_check
#       cell_anno[(count+1):(count+length(cells_to_check))] <- cell_types[cell_number_to_use[i]]
...
# }
################################################################################################
#' Find the cell types of the neighbors for cells_to_check
#' @export
neighbor_cell_type <- function(CelestaObj,cell_type_num,round,cells_to_check){
  ### Only has information for cells to check
  nb_list <- CelestaObj@nb_list
  cell_type_assignment <- CelestaObj@current_cell_type_assignment[,round]
  same_type_nb <- matrix(rep(list(),length(cell_type_num)*length(cells_to_check)),
                         nrow=length(cells_to_check),ncol=length(cell_type_num))
  row.names(same_type_nb) <- cells_to_check
  colnames(same_type_nb) <- cell_type_num
  for(j in 1:length(cells_to_check)){
    current_cell_ID <- cells_to_check[j]
...
#' @export
get_dist_from_nearest_assigned_cells <- function(CelestaObj,cell_type_num,cells_to_check,
                                                 cells_with_ID,round){
  ...
  current_cell_type_assignment <- CelestaObj@current_cell_type_assignment
  dist_nearest_assigned_cell <- matrix(nrow = length(cells_to_check),
                                       ncol = length(cell_type_num))
  colnames(dist_nearest_assigned_cell) <- cell_type_num
  for(i in 1:dim(dist_nearest_assigned_cell)[1]){
    cell_to_check <- cells_to_check[i]
    matching <- match(all_cell_nb_in_circle[[cell_to_check]],cells_with_ID)
    index <- matching[which(is.na(matching)==FALSE)]
    if(length(index)==0){
      
    }else{
      nb_cell_with_ID <- cells_with_ID[index]
...
#' @export
cell_prob <- function(CelestaObj,cell_type_num,cells_to_check,round){
  ...
  for(i in 1:length(cells_to_check)){
    cell_ID_to_check <- cells_to_check[i]
...
update_prior_matrix <- function(CelestaObj,round,cell_type_num){
  ...
  current_cell_type_assignment <- CelestaObj@current_cell_type_assignment
  all_marker_pro_matrix <- CelestaObj@marker_activation_prob
...
plot_cells_any_combination <- function(cell_type_assignment_to_plot,CelestaObj,
  ...
  for(i in 1:length(cell_number_to_use)){
    cells_to_check <- which(current_cell_type_assignment == cell_number_to_use[i])
    cell_index[(count+1):(count+length(cells_to_check))] <- cells_to_check
    cell_anno[(count+1):(count+length(cells_to_check))] <- cell_types[i]
    count <- count + length(cells_to_check)
  }
  df_plot <- data.frame(x=coords[cell_index,1],
                        y=coords[cell_index,2],
...
plot_exp_prob <- function(CelestaObj,size_to_use=1,width_to_use=5,height_to_use=4){
  coords <- CelestaObj@coords
  marker_exp_prob <- CelestaObj@marker_activation_prob
  prior_marker_info <- CelestaObj@prior_info
...
assign_cell_main <- function(CelestaObj,max_iteration=10,cell_change_threshold=0.01,
  ...
  for(i in 1:total_rounds){
    ...
    cells_to_check <- find_cells_to_check(CelestaObj,round)
    number_of_cells_to_find_identity <- length(cells_to_check)
    print(number_of_cells_to_find_identity)
    ### Get scoring function 
    CelestaObj <- scoring_function(CelestaObj,round,cells_to_check,cell_type_num)
    ...
    CelestaObj <- cell_type(CelestaObj,cell_type_num,cells_to_check,round,
    ...
    ### Find cells to check
    cells_to_check <- find_cells_to_check(CelestaObj,round)
    cells_with_ID <- find_cells_with_ID(CelestaObj,round)
    #############
    ### Calculate beta
    CelestaObj <- neighbor_cell_type(CelestaObj,cell_type_num,round,cells_to_check)
    CelestaObj <- get_dist_from_nearest_assigned_cells(CelestaObj,cell_type_num,
                                                  cells_to_check,cells_with_ID,round)
    ...
    CelestaObj <- cell_prob(CelestaObj, cell_type_num,cells_to_check,round)
    old_cell_assignment <- CelestaObj@current_cell_type_assignment[,round]
